...
horizontal cell gaps. The core is [50, 75.6]-[960, 982.8] um, retaining 50 um
of left and 40 um of right side margin for ordinary pin access and the power
trunks. Five row intervals retain an empty 75.6 um site row; the final pair
is adjacent, putting the cell stack between y=75.6 and 982.8 um.
...
router/stream-out correlation errors without modifying the PDK or final GDS
rules. The `power_channels.json` configuration places each row's M1 buses only
4.0 um from the GND/VDD pin centerline, places the GND trunk at x=54 um, and
uses a 40 um right-edge inset for VDD. All cell supplies are connected through
these M1 channels.
...
The local generic regression is expected to report zero OpenROAD detailed-route
violations, no hard KLayout drawing-DRC items, connected extracted instances,
and a matching strict LVS contract. The official `run_IP62.drc` deck is not
part of this local result and requires a separately supplied foundry-compatible
container image. DRC/LVS success is not timing, IR-drop, electromigration, or
frame/pad signoff.

Timing constraints remain deliberately relaxed because the current TR-1um
Liberty data is placeholder characterization.
