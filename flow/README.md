...
`pya` runtime, run the wrapper from the same local LibreLane `nix-shell`
session rather than invoking a system Python interpreter:

```bash
cd "$HOME/Documents/TR-1um"
klayout -b -zz -r /tmp/run_gen_macro.py
```

The wrapper should set its input/output paths to the repository's macro GDS
and LEF. The source macro is preserved as
`CMC_S_NMOS_B_X1_Y1.source.gds`.
