...
empty M2 corridor and is checked by the same drawing DRC used for signoff.
...
#### Compact UART channel-power experiment

`tr1um_uarttx` is a 29.4912 MHz, 115200-baud transmitter in a 1000 by 1000 um
macro.  It continuously emits `UART` eight times (32 bytes).  The RTL uses one
14-bit phase counter; its low eight bits divide the clock, four bits select one
of 16 UART slots, and two bits select the character.  Direct bit decoding
avoids a character ROM, shift register, and separate bit/character counters.
The qualified run maps to 63 standard-cell instances in six occupied 75.6 um
rows with 10 um cell gaps.

Run the complete generic flow from the LibreLane development shell:

```bash
./flow/designs/run_digital_tests.sh \
  --tag uart-compact-1000-final-1 tr1um_uarttx
```

The design-local `manual_place_access.tcl` spreads instances across all six
rows.  `postprocess_gds.py` joins the rectangular WN bands with one left-side
WN spine, producing a single common n-well network without changing the
standard-cell masters.  The generic streamout and connected-core flow invokes
that postprocessor only when it exists in a design directory, so other designs
are unaffected.  The connected-power stage routes GND and VDD separately,
then performs OpenROAD detailed-route DRC, foundry KLayout drawing DRC, a raw
M1/V1/M2 supply-connectivity check, and strict transistor-level LVS.

The reference run reports zero detailed-route and drawing-DRC violations,
one conductive component for every GND terminal, one for every VDD terminal,
and `Congratulations! Netlists match` for all 63 instances and 14 cell types.
Its four external pins are deliberately integration-oriented: `clk` is on the
west edge, `tx` is on the east edge, and GND/VDD are on the south edge.  The
logical GND name is normalized to the process LVS name VSS in the GDS/SPICE
views.

The output directory is a reusable hard-macro bundle:

- `tr1um_uarttx.connected_power.gds` has real metal labels for `tx`, `clk`,
  VDD, and VSS.
- `tr1um_uarttx.macro.lef` is the abstract physical view for a parent
  LibreLane/OpenROAD design.
- `tr1um_uarttx.strict.cir` is the independently generated SPICE/LVS view.
- `flow/designs/tr1um_uarttx/macro/tr1um_uarttx.blackbox.v` is the synthesis
  black box, and the adjacent `.sym` is an xschem symbol with SPICE pin order
  `tx clk VDD VSS`.
...
For a larger hardened hierarchy, put the UART GDS/LEF in the parent's macro
views and instantiate its black-box Verilog.  Pass the UART contract after the
parent output directory; multiple macro contracts may be listed:

```bash
./flow/run_core_lvs.sh \
  parent.gds parent.def parent.pnl.v parent_top /tmp/parent-lvs \
  uart-build/tr1um_uarttx.strict.cir
```

The builder checks the complete parent physical/Verilog instance population,
recursively includes the macro's contract, stamps all parent DEF ports onto
the GDS label layers, and runs strict-port LVS.  In xschem, place
`tr1um_uarttx.sym`, connect `tx clk VDD VSS`, and include/concatenate
`tr1um_uarttx.strict.cir` with the parent SPICE netlist before running the same
KLayout LVS deck.  Do not use the behavioral RTL as the LVS schematic: the
strict `.cir` file is the transistor-level physical contract.

This path is regression-tested both on the UART itself and on a synthetic
`uart_parent` GDS whose only child is the hardened UART.  Both comparisons run
in strict-port mode and report `Congratulations! Netlists match`.

This topology grows linearly with the number of cells, but it is not yet a
general power-grid generator.  The power-routing helper and UART well joiner
both fail closed when their placement assumptions are not met.  A larger
design should increase die area and row count, retain routing channels, and
re-run every check.  The 1.8 um M1 bus width is the process drawing minimum
used by this DRC experiment; tapeout use also requires an IR-drop and
electromigration assessment and may need wider or parallel buses.  The common
WN geometry also requires foundry review before tapeout.  This remains a
core-only result and does not solve the foundry frame-pad transition.
...
The final framed gate uses the saved routed GDS and strict contract:

```bash
bash flow/scripts/signoff/run_tr1um_signoff.sh \
  flow/signoff/tr_1um_mixed_counter_routed.gds \
  tr_1um_mixed_counter \
  flow/signoff/tr_1um_mixed_counter.cir \
  /tmp/tr1um-final-signoff
```

The strict contract is generated from the routed PNL Verilog, analog CDL,
and independent frame extraction by
`flow/scripts/signoff/build_strict_mixed_contract.py`. It must not be
generated by copying the routed physical extraction.

## Final mixed-signal signoff evidence

The routed mixed-signal design is the signoff artifact. The LibreLane run
`flow/designs/tr1um_mixed_counter/runs/mixed-beol-esc1` routes the analog
macro through its BEOL escape ring (M2 landing pads) with the access-cell
digital library. Verified metrics from `final/metrics.json`:

| Metric | Result |
|---|---:|
| OpenROAD route DRC errors | 0 |
| KLayout drawing DRC errors | 0 |
| Standard-cell + macro instances | 13 |
| Setup WNS/TNS | 4980.94 / 0 |
| Hold WNS/TNS | 8.36 / 0 |
| Unmapped cells | 0 |

The routed GDS is framed (2500 x 2500um, dbu 0.001, `OSS_FRAME` hierarchy)
and passes the complete signoff gate with a **strict, non-circular LVS
contract** built from the routed PNL Verilog, the analog macro CDL, and an
...
Direct evidence (last full run, `RC=0`):
...
macro GND escape through an empty, DRC-checked M2 corridor; it is not a
disconnected-pin waiver and does not alter the logical netlist.
...
The saved routed GDS contains one long M2 bridge path; direct drawing DRC is
0. The current framed rerun returns `RC=0`, strict LVS matches, IP62 mask DRC
is 0, and the RCX estimate is generated.
...
Reproduce the full local gate from a local LibreLane shell with:
...
The current verified result is `RC=0`, with zero hard drawing DRC items,
strict LVS match, generated MDP, zero hard IP62 mask DRC items, and a
DEF-derived 20-net engineering SPEF.
...
```bash
./flow/designs/run_digital_tests.sh --tag my-core tr1um_uarttx
