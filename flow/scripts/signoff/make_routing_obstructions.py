...
EXTRA_OBS = {
    # Constant-output tie pins are physically joined to their corresponding
    # supply by the normalized GDS view. Keep unrelated signal routes out of
    # the full M1 output straps, including the segment between the supply rail
    # and the nominal LEF pin rectangle.
    "TIEHI": [("M1", 2.5, 45.5, 5.5, 62.6)],
    "TIELO": [("M1", 2.5, -1.5, 5.5, 17.1)],
    # Foundry drawing-DRC correlation found gate edges not fully represented
    # by the generated V1-only FEOL abstraction. Keep the route-access
    # exceptions explicit and cell-local.
    "NOR2": [("V1", 7.0, 5.0, 14.0, 53.0)],
...
def main() -> int:
    ...
    for line in lines:
        ...
        if in_obs and words == ["END"]:
            ...
            in_obs = False
        elif len(words) >= 2 and words[0] == "END" and words[1] == macro:
            extra_obs = EXTRA_OBS.get(macro, [])
            if extra_obs:
                output.append("  OBS")
                for obs_layer, x0, y0, x1, y1 in extra_obs:
                    output.append(f"    LAYER {obs_layer} ;")
                    output.append(f"      RECT {x0:.3f} {y0:.3f} {x1:.3f} {y1:.3f} ;")
                output.append("  END")
        output.append(line)
