...
class Module(object):
    ...
    def __init__(self, object: Union[odb.dbBlock, odb.dbInst]) -> None:
        ...
        is_top_block = isinstance(object, odb.dbBlock)
        terminals = object.getBTerms() if is_top_block else object.getITerms()
        power_found = False
        ground_found = False
        for terminal in terminals:
            signal_type = terminal.getSigType()
            # OpenROAD emits top-level DEF pins VDD/GND with USE SIGNAL in
            # this flow, even though their SPECIALNETS are POWER/GROUND.
            # Resolve the authoritative top-level classification from the net
            # so the checker reports physical connectivity rather than a
            # synthetic '<ANY POWER PIN>' failure.
            if terminal.getName().upper() == "GND" or (terminal.getNet() is not None and terminal.getNet().getName().upper() == "GND"):
                signal_type = "GROUND"
            elif is_top_block and terminal.getNet() is not None:
                net_signal_type = terminal.getNet().getSigType()
                if net_signal_type in ("POWER", "GROUND"):
                    signal_type = net_signal_type
        ...
        if is_top_block and not power_found:
            print(
                f"[ERROR] Top-level block {object.getName()} has no power pins",
                file=sys.stderr,
            )
            self.ports["<ANY POWER PIN>"] = Port("INOUT", "POWER", False)
        if is_top_block and not ground_found:
            print(
                f"[ERROR] Top-level block {object.getName()} has no ground pins",
                file=sys.stderr,
            )
