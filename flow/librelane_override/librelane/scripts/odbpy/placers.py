    ...
    for instance, info in data.items():
        ...
        for inst in reader.block.getInsts():
            if inst.getName() == name_escaped:
                ...
                # OpenROAD rejects orientation changes on FIRM instances.
                # Manual placement is intended to position, not reorient, fixed
                # instances; only change orientation while the instance is editable.
                if not inst.isFixed():
                    inst.setOrient(lef_rot_to_oa_rot(orientation))
