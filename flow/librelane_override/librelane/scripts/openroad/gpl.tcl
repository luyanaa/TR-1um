...
# The access-cell SCLs use a 75.6 um site and are numerically unstable in
# RePlAce for this sparse 1 um test design. Use the verified legal placement
# for either derived access library.
if { [info exists ::env(STD_CELL_LIBRARY)] && ($::env(STD_CELL_LIBRARY) == "TR-1um_stdcell_access" || $::env(STD_CELL_LIBRARY) == "TR-1um_stdcell_access_power") && [file exists $::env(DESIGN_DIR)/manual_place_access.tcl] } {
    puts "\[INFO\] Using access-cell manual placement instead of RePlAce"
    source $::env(DESIGN_DIR)/manual_place_access.tcl
    write_views
    exit_unless_gui
}
