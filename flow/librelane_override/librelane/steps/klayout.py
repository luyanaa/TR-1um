...
class StreamOut(KLayoutStep):
    ...
    def run(self, state_in: State, **kwargs) -> Tuple[ViewsUpdate, MetricsUpdate]:
        ...
        if self.config["DESIGN_NAME"] == "tr1um_mixed_counter":
            bridge = os.path.abspath(
                os.path.join(self.config["DESIGN_DIR"], "..", "..", "scripts", "signoff", "add_gnd_bridge.py")
            )
            bridged = os.path.join(self.step_dir, f"{self.config['DESIGN_NAME']}.gnd_bridge.gds")
            self.run_pya_script(
                [sys.executable, bridge, "--input", klayout_gds_out, "--output", bridged],
                env=env,
            )
            shutil.copy(bridged, klayout_gds_out)
